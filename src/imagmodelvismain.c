...
int imagmodel(int argc, char **argv )
{
 int status = 0;
 ...
 getpar_modelvis( &proj );
 prep_modelvis_display( & proj );

 do{
 buffer_range( &(proj.dispBuf), proj.redundant_baselines+1, proj.chans );
 loop = imag_modelvis( &proj, &plot );
 }while(loop);
