...
int plotHess(int argc, char **argv )
{
 PlotParType plot;
 plot.driver = dev;
 ...
 { plot.driver = 'X';
   xsplot_Hessian_and_covar( &plot, solfilename );
 }
 else if ( plot.driver == 'P' || plot.driver == 'p' )
 { plot.driver = 'P';
   psplot_Hessian_and_covar( &plot, solfilename );
 }
...
int main(int argc, char **argv )
{
  ...
  optUsage(Usage);
  optTitle("Generates plots of the Hessian matrix. \n");
  optMain(plotHess);
  opt(&argc,&argv);

  return plotHess(argc, argv);
}
