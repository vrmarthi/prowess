...
int calib(int argc, char **argv )
{
 int status = 0;
 int ch, del_iter = 0;
 long int rec, nrecs, solno = 0, gcount;
 fitsfile *fin, *fout;
 ...
 init_flagmask( &proj );
 //make_flagmask( fin, &proj );

 if( N == -1 ) N = proj.chans / 2;
 proj.chan2cal = N;
 //check_chan2cal( fin, &proj );
 ...
 setup_itersol( &proj );
 if( modelFile !=NULL ) read_model_visibilities( &proj, modelFile );
 ...
 { force_model( &proj );
   ...
   if(del_iter == MAXITER) fprintf(finfo, "%s Reached max. no. of iterations.\n", info); 
 }

 write_gaintab( &proj );

 /*Bandpass calibration of the calibrator */
 ...
 { del_iter = 0;
   ...
   write_bpasstab( &proj );
  fprintf(finfo, "%s Channel [ %3d / %3d ] converged in %4d iterations\n", info, proj.chan2cal+1, proj.chans, del_iter);
 }
 ...
 { /*Gain calibration g(t,p) */
   ...
   fprintf(finfo, "%s Bandpass [ %ld / %d ] converged in %d iterations\r", info, solno+nrecs, proj.nrecs, proj.iter);

   /*Apply calibration solutions and write to FITS */
   apply_solutions( &proj );
   //fprintf(finfo, "%s Applied solutions to [ %ld / %d ] records\r", info, solno, proj.nrecs);
 ...
 fgain = fopen("gain.dat" ,"w");
 write_gainfile ( &proj, fgain );
 fclose(fgain);

 fbpass = fopen("bpass.dat" ,"w");
 write_bpassfile ( &proj, fbpass );
 fclose(fbpass);
 ...
 do{
   fprintf(fprompt, "%s BSOL # to quit / display [ 0 / 1 - %d ] : ", prompt, proj.nrecs+1 );
   scanf("%d", &rec2disp);
   if(rec2disp < 1 ) 
