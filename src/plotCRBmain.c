...
int plotCRB(int argc, char **argv )
{
 PlotParType plot;
 plot.driver = dev;
 ...
 { plot.driver = 'X';
   xsplot_Fisher_and_CRB( &plot, solfilename );
 }
 else if ( plot.driver == 'P' || plot.driver == 'p' )
 { plot.driver = 'P';
   psplot_Fisher_and_CRB( &plot, solfilename );
 }
...
int main(int argc, char **argv )
{
  ...
  optUsage(Usage);
  optTitle("Generates plots of the CRB matrix. \n");
  optMain(plotCRB);
  opt(&argc,&argv);

  return plotCRB(argc, argv);
}
