...
int FITSapplcal(int argc, char **argv )
{
 ...
 { for(i = 0; i < proj.redundant_baselines; i++)
    fprintf(finfo, "%s Ch : %d Re : %f Im : %f\n", info, ch, proj.model_vis[ch][i].r, proj.model_vis[ch][i].i);
 }
 getchar();
 */
 proj.preint = 1;
 ...
 { /*Gain calibration g(t,p) */
   ...
   /*for(ch = 0 ; ch < proj.chans;ch++)
 { for(i = 0; i < proj.redundant_baselines; i++)
	 fprintf(finfo, "%s Rec : %d Ch : %d Re : %f Im : %f\n", info, solno, ch, proj.model_vis[ch][i].r, proj.model_vis[ch][i].i);
	 }*/
   //getchar();
 write_gaintab( &proj, fgain );
 //apply_model( &proj );
